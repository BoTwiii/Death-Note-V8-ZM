...
#define mSLIDE_COLOR = color(0xfa0000);
...
SetCursor(menu = "none", value = 0, oldValue = value)
{
    ...
    if(self.submenu == menu && delta)
    {
        ...
        new_elem      = Text("", -100, 60 + (add_index * 25) + mHUD_Y_OFF, "default", 1.5, mTEXT_COLOR, 0, 2, "CENTER", "TOP_RIGHT");
...
SetMenuOpen(isOpen)
{
    if(bool(isOpen))
    {
        ...
        self.notifyLuiData = self thread LUI_createRectangle( 2, 1070, 45, 120, 120, (255, 255, 255), "menu_zm_rank_5_ded", 1,0);
    }
    else
    {
        self.submenu = "none";
        ...
        self CloseLUIMenu(self.notifyLuiData);
    }
}
...
DrawMenu(animStyle = mANIM_NONE)
{
    ...
    self.mbg    = self Icon(mBG_SHADER, -10 + 10, 0, 200, 1000, mBG_COLOR, mBG_ALPHA, 0, "TOP_RIGHT", "TOP_RIGHT");
    self.mTitle = self Text(GetMenu().o, -100, 20, "default", 2.0, mTEXT_COLOR, mTEXT_ALPHA, 1, "CENTER", "TOP_RIGHT");
    ...
    left = self Icon("white", 0, 0, 2, 1000, inversecolor(mSLIDE_COLOR), mSLIDE_ALPHA, 1, "TOP_LEFT", "TOP_LEFT");
    left hud::SetParent(self.mbg);
    left ScaleOverTime(mSCROLL_DELAY, 2, 1000);
    ...
    for(i = 0; i < GetMaxTextIndex(); i++) //i = i + 1
    {
        ...
        self.mText[i] = self Text(GetOption(offset + i).o, -100, 60 + (i * 25) + mHUD_Y_OFF, "default", 1.5, mTEXT_COLOR, mTEXT_ALPHA, 2, "CENTER", "TOP_RIGHT");
